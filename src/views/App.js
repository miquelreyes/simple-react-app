...
import {
	...
	updateSearchText,
	toggleAddForm,
} from "../redux/actions/holidaysActions";
import {
	selectHolidays,
	selectFilteredHolidays,
	selectLoading,
	selectShowForm,
} from "../redux/selectors/holidaysSelectors";
...
const App = ({ className }) => {
	...
	const options = useCallback(
		holidays.map(holiday => {
			return { value: holiday.name };
		}),
		[holidays]
	);
	const toggleFormOnClick = useCallback(
		() => dispatch(toggleAddForm()),
		[dispatch]
	);
	const addOnSubmit = useCallback(
		holiday => dispatch(addHolidayRequest(holiday)),
		[dispatch]
	);
	const fetchOnClick = useCallback(
		() => dispatch(fetchHolidaysRequest()),
		[dispatch]
	);
	...
	return (
		<div className={className}>
			...
			/>
			<StyledButton onClick={toggleFormOnClick}>
				{showingForm ? "Hide form" : "Add holiday"}
			</StyledButton>
			{showingForm && <StyledAddHolidayForm onSubmit={addOnSubmit} />}
			<FetchButton onClick={fetchOnClick} text={"Fetch data again"} />
			<StyledCalendar holidays={holidays} />
			<StyledHolidayList loading={loading} holidays={filtered} />
		</div>
	);
};
